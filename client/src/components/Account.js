  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (success) {
        ...
        // Assuming userData is directly available and token is set in cookies
        if (token) {
          console.log(token);
          Cookies.set('token', token); // Store token in cookies for 7 days
        
        }
        // Dispatch the login action with userData
        dispatch(login(userData));
        
        // Navigate based on whether it's a signup or login
        navigate(isSignup ? '/account' : '/');
      } else {
        // Handle error case
        setErrors({ ...errors, general: message || 'An error occurred. Please try again.' });
      }
